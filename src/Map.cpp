...
void Map::LoadMap(const std::string &filename) {
    ...
    if (mapFile.is_open()) {
        ...
        while (std::getline(mapFile, line)) {
            if (!StringUtils::removeSpaces(line).empty()) {
                if (line.rfind(' ', 0)) {
...
bool Map::canTravel(uint16_t tileX, uint16_t tileY) const {
...
void Map::setPropertyMapAtlas(std::string &object_data) {
    ...
    while ((pos = object_data.find(delimiter)) != std::string::npos) {
        ...
        map_atlas[std::stoi(token.substr(0, splitterIndex))] = TextureManager::LoadTexture(filename);
