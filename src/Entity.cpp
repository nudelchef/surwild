...
bool Entity::canMove(Direction direction) const {
    switch (direction) {
        case Direction::UP:
...
void Entity::updateMovement() {
    ...
    if (distanceLeft <= 0) {
        ...
        // send the message to the server to update the player's position using the static client from Game class
        Game::client->send(sstream.str());
    }
}
